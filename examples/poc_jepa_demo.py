...
async def demo_media_processing(runner: JEPARunner, image_path: str):
    ...
    print(f"Physics Embedding: [{physics_emb[0]:.4f}, {physics_emb[1]:.4f}, ..., {physics_emb[-1]:.4f}]")
...
def demo_simulation(runner: JEPARunner):
    ...
    actions = [
        {"type": "MOVE", "target": "robot_arm", "target_position": {"x": 0.0, "y": 0.0, "z": 0.2}},
        {"type": "GRASP", "target": "red_block"},
        {"type": "MOVE", "target": "robot_arm", "target_position": {"x": 0.3, "y": 0.0, "z": 0.2}},
    ...
    for state in result.imagined_states:
        ...
        for entity in state.entities:
            if entity.position:
                pos = entity.position
                print(f"    {entity.id}: ({pos['x']:.3f}, {pos['y']:.3f}, {pos['z']:.3f})")
...
def demo_backend_comparison():
    ...
    entities = [Entity(id="test_obj", type="object", position={"x": 0.0, "y": 0.0, "z": 0.1})]
    ...
    print("-" * 70)
    print(f"{'Backend Class':<30} {stub_runner._backend.__class__.__name__:<20} {poc_runner._backend.__class__.__name__:<20}")
    print(f"{'Model Version':<30} {stub_result.model_version:<20} {poc_result.model_version:<20}")
    print(f"{'Overall Confidence':<30} {stub_result.overall_confidence:.4f}{'':<16} {poc_result.overall_confidence:.4f}")
    print(f"{'# Processes':<30} {len(stub_result.imagined_processes):<20} {len(poc_result.imagined_processes):<20}")
    print(f"{'# States':<30} {len(stub_result.imagined_states):<20} {len(poc_result.imagined_states):<20}")
...
async def main():
    ...
    print("=" * 70)
    print("\nNext steps:")
    print("  1. Run with PoC backend: JEPA_BACKEND=poc python examples/poc_jepa_demo.py")
