...
class TestPoCJEPABackendIntegration:
    ...
    def test_simulation_with_actions(self, poc_backend):
        ...
        result = poc_backend.simulate(
            context, k_steps=2, assumptions=["robot is ready"]
        )
...
class TestJEPARunnerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_runner_process_media_sample(
        self, minimal_checkpoint, tmp_path, monkeypatch
    ):
