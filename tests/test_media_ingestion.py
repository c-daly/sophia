...
def test_extract_image_metadata(media_storage_service, sample_image_file, temp_storage_dir):
...
class TestMediaIngestEndpoint:
    ...
    def test_ingest_media_requires_authentication(self, mock_hcg, mock_ingestion, client):
    ...
    @patch('sophia.api.app._media_ingestion')
    @patch('sophia.api.app._hcg_client')
    def test_ingest_media_validates_media_type(self, mock_hcg, mock_ingestion, client, auth_headers):
        """Test that /ingest/media validates media_type field."""
        files = {"file": ("test.jpg", b"fake image data", "image/jpeg")}
        data = {"media_type": "invalid_type"}
        
        response = client.post("/ingest/media", files=files, data=data, headers=auth_headers)
        assert response.status_code == 422

    @patch('sophia.api.app._media_ingestion')
    @patch('sophia.api.app._hcg_client')
    def test_ingest_media_success(self, mock_hcg, mock_ingestion, client, auth_headers, sample_image_file):
    ...
    @patch('sophia.api.app._media_ingestion')
    @patch('sophia.api.app._hcg_client')
    def test_ingest_media_handles_validation_error(self, mock_hcg, mock_ingestion, client, auth_headers):
        """Test that /ingest/media handles validation errors."""
        mock_ingestion.ingest_media = AsyncMock(side_effect=ValueError("Invalid file extension"))
    ...
    @patch('sophia.api.app._media_ingestion')
    def test_ingest_media_returns_503_when_service_unavailable(self, mock_ingestion, client, auth_headers):
...
class TestMediaSamplesListEndpoint:
    ...
    @patch('sophia.api.app._media_ingestion')
    @patch('sophia.api.app._hcg_client')
    def test_list_samples_requires_authentication(self, mock_hcg, mock_ingestion, client):
    ...
    @patch('sophia.api.app._media_ingestion')
    @patch('sophia.api.app._hcg_client')
    def test_list_samples_returns_paginated_results(self, mock_hcg, mock_ingestion, client, auth_headers):
    ...
    @patch('sophia.api.app._media_ingestion')
    @patch('sophia.api.app._hcg_client')
    def test_list_samples_supports_filtering(self, mock_hcg, mock_ingestion, client, auth_headers):
        """Test that /media/samples supports filtering by media_type."""
        mock_ingestion.list_media_samples = AsyncMock(
            return_value=MediaSamplesListResponse(samples=[], total=0, limit=50, offset=0)
        )
...
class TestMediaSampleDetailEndpoint:
    ...
    @patch('sophia.api.app._media_ingestion')
    @patch('sophia.api.app._hcg_client')
    def test_get_sample_returns_sample_details(self, mock_hcg, mock_ingestion, client, auth_headers):
    ...
    @patch('sophia.api.app._media_ingestion')
    @patch('sophia.api.app._hcg_client')
    def test_get_sample_returns_404_when_not_found(self, mock_hcg, mock_ingestion, client, auth_headers):
