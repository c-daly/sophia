...
class PlanResponse(BaseModel):
    ...
    plan_id: Optional[UUID] = Field(
        default=None, description="Identifier of the persisted plan"
    )
    status: Optional[StrictStr] = Field(
        default=None, description="Planner status at response time"
    )
    submitted_at: Optional[datetime] = None
    diagnostics: Optional[List[StrictStr]] = Field(
        default=None, description="Human-readable planning notes or warnings"
    )
    states: Optional[List[CWMState]] = Field(
        default=None,
    ...
    def status_validate_enum(cls, value):
        ...
        if value not in set(["accepted", "in_progress", "completed", "failed"]):
            raise ValueError(
                "must be one of enum values ('accepted', 'in_progress', 'completed', 'failed')"
            )
        return value
