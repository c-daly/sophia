...
- When you start work on an issue that lives on the LOGOS workspace project, move its card to *In Progress* (and adjust any `status/*` label). Once a PR is open and waiting for review/merge, move it to *PR awaiting merge*. After the change lands, move it to *Done* so the shared board remains accurate.
